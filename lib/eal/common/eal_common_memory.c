...
parse_heap_mem_stats(struct rte_heap_mem_stats *heap_stats)
{
	...
	if (fp == NULL) {
		RTE_LOG_LINE(DEBUG, EAL, "Error: Failed to open memory stream");
		return -1;
	}
	...
	if (ret != 0) {
		RTE_LOG_LINE(DEBUG, EAL, "Error: malloc_info returned error");
		return -1;
	}
		...
		if (buf[i] == '<') {
			...
			if (strncmp(tag, "heap", strlen("heap")) == 0) {
				...
				if (mem_index >= MAX_SYS_MEM_ARENAS) {
					RTE_LOG_LINE(DEBUG, EAL, "Memory arena "
			   ...
			   struct rte_tel_data *d)
{
	...
	if (arena_id >= heap_mem_stats.num_active_arena) {
		RTE_LOG_LINE(DEBUG, EAL, "Memory arena exceeded max limit: %d",
			MAX_SYS_MEM_ARENAS);
		return -1;
	}
