		...
		int (*worker)(void *))
{
	...
	if (opt->tx_first) {
		...
		setup_pkt_udp_ip_headers(&ip_hdr, &udp_hdr,
					 TX_DEF_PACKET_LEN -
		...
		RTE_ETH_FOREACH_DEV(eth_port_id) {
			...
			for (i = 0; i < opt->tx_first; i++) {
				...
				struct rte_mbuf *mbuf =
					rte_pktmbuf_alloc(*t->pool);

				mbuf->port = eth_port_id;
				mbuf->data_len = TX_DEF_PACKET_LEN;
				mbuf->pkt_len = TX_DEF_PACKET_LEN;

				/* Copy Ethernet header */
				rte_memcpy(rte_pktmbuf_mtod_offset(mbuf, char *,
								   0),
					   &eth_hdr,
					   sizeof(struct rte_ether_hdr));
				...
				pkt_udp_hdr = rte_pktmbuf_mtod_offset(mbuf, struct rte_udp_hdr *,
						sizeof(struct rte_ipv4_hdr) +
						sizeof(struct rte_ether_hdr));
				pkt_udp_hdr->src_port = rte_cpu_to_be_16(UDP_SRC_PORT + i);
				pkt_udp_hdr->dst_port = rte_cpu_to_be_16(UDP_SRC_PORT + i);
	...
	while (t->done == false) {
		...
		if ((new_cycles - perf_cycles) > perf_sample) {
...
#ifdef RTE_EXEC_ENV_ASIM
			float mpps = (float)(curr_pkts - prev_pkts);
#else
			...
			++samples;
#ifdef RTE_EXEC_ENV_ASIM
			printf(CLGRN "\r%.3f pps avg %.3f pps" CLNRM, mpps,
			       total_mpps / samples);
#else
		...
		struct rte_event_port_conf prod_conf)
{
	...
	if (opt->ena_vector) {
		...
		nb_elem += evt_nr_active_lcores(opt->wlcores) * 16;
		vector_pool = rte_event_vector_pool_create(
			"vector_pool", nb_elem, 16, opt->vector_size,
			opt->socket_id);
		if (vector_pool == NULL) {
