				      ...
				      enum rte_crypto_op_sess_type sess_type, void *mdata)
{
	...
	if (op_type == RTE_CRYPTO_OP_TYPE_SYMMETRIC) {
		if (sess_type == RTE_CRYPTO_OP_SECURITY_SESSION) {
			struct cn10k_sec_session *sec_sess = get_sec_session_private_data(sess);
			sec_sess->qp = qp;
			sec_sess->inst.w2 = w2;
...
static inline int
ca_lmtst_vec_submit(struct ops_burst *burst, struct vec_request vec_tbl[], uint16_t *vec_tbl_len, const bool is_sg_ver2)
{
