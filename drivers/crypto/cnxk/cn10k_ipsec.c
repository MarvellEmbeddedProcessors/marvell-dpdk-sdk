			   ...
			   struct rte_security_session *sess)
{
	...
	sec_sess = get_sec_session_private_data(sess);
	sa = &sec_sess->sa;
	out_sa = &sa->out_sa;
			   ...
			   struct rte_security_session *sess)
{
	...
	if (ipsec_xfrm->direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS)
		return cn10k_ipsec_inb_sa_create(roc_cpt, &qp->lf, ipsec_xfrm, crypto_xfrm, sess);
	else
		return cn10k_ipsec_outb_sa_create(roc_cpt, &qp->lf, ipsec_xfrm, crypto_xfrm, sess);
}
