...
static const uint64_t mark_flag[NIX_TM_MARK_MAX] = {
	[NIX_TM_MARK_VLAN_DEI] = NIX_TM_MARK_VLAN_DEI_EN,
	[NIX_TM_MARK_IPV4_DSCP] = NIX_TM_MARK_IP_DSCP_EN,
	[NIX_TM_MARK_IPV4_ECN] = NIX_TM_MARK_IP_ECN_EN,
	[NIX_TM_MARK_IPV6_DSCP] = NIX_TM_MARK_IP_DSCP_EN,
	[NIX_TM_MARK_IPV6_ECN] = NIX_TM_MARK_IP_ECN_EN,
};
...
static bool
nix_tm_update_markfmt(struct nix *nix, enum nix_tm_mark type, int mark_yellow,
		      int mark_red)
{
...
nix_tm_mark_init(struct nix *nix)
{
	...
	nix_tm_update_markfmt(nix, NIX_TM_MARK_VLAN_DEI, 0, 0);
	nix_tm_update_markfmt(nix, NIX_TM_MARK_IPV4_DSCP, 0, 0);
	nix_tm_update_markfmt(nix, NIX_TM_MARK_IPV4_ECN, 0, 0);
	nix_tm_update_markfmt(nix, NIX_TM_MARK_IPV6_DSCP, 0, 0);
	nix_tm_update_markfmt(nix, NIX_TM_MARK_IPV6_ECN, 0, 0);
exit:
	return rc;
}

int
roc_nix_tm_mark_config(struct roc_nix *roc_nix, enum nix_tm_mark type,
...
uint64_t
roc_nix_tm_get_mark_format(struct roc_nix *roc_nix, uint64_t *flags)
{
	struct nix *nix = roc_nix_to_nix_priv(roc_nix);
