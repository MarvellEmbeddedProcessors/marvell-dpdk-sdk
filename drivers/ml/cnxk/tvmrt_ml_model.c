...
tvmrt_ml_model_json_parse(struct cnxk_ml_model *model)
{
	...
	if (json_parsed == NULL) {
		tvmrt_ml_param_names_free(&param_names);
		plt_err("TVM runtime: Failed to parse metadata, model_id = %u", model->model_id);
		return -EINVAL;
	}
	...
	if (!json_is_array(json_nodes)) {
		ret = -EINVAL;
		plt_err("TVM runtime: Failed to parse metadata, model_id = %u, error = %d",
			model->model_id, ret);
		goto error;
	}
...
tvmrt_ml_model_info_set(struct cnxk_ml_dev *cnxk_mldev, struct cnxk_ml_model *model)
{
	...
	for (i = 0; i < info->nb_inputs; i++) {
		...
		input[i].dtype = model->tvmrt.info.input[i].dtype;
		input[i].dsize = model->tvmrt.info.input[i].nb_elements *
				 rte_ml_io_type_size_get(model->tvmrt.info.input[i].dtype);
	}
	...
	for (i = 0; i < info->nb_outputs; i++) {
		...
		output[i].dtype = model->tvmrt.info.output[i].dtype;
		output[i].dsize = model->tvmrt.info.output[i].nb_elements *
				  rte_ml_io_type_size_get(model->tvmrt.info.output[i].dtype);
	}
