		...
		uint16_t max_queues)
{
	...
	priv->ppio_params.rate_limit.rate_limit_enable =
		port_cfg->rate_limit_enable;
	...
	for (i = 0; i < max_queues; i++) {
		...
		params->rate_limit.rate_limit_enable =
			port_cfg->outq[i].rate_limit_enable;
		params->rate_limit.rate_limit_params =
			port_cfg->outq[i].rate_limit_params;
	}
