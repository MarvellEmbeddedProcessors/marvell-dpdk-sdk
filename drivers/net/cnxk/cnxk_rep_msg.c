...
receive_control_message(int socketfd, void *data, uint32_t len)
{
	...
	rec++;
	plt_rep_dbg("Packet %ld Received %ld bytes over socketfd %d", rec, size, socketfd);
...
send_message_on_socket(int socketfd, void *data, uint32_t len, int afd)
{
	...
	sent++;
	plt_rep_dbg("Sent %ld packets of size %d on socketfd %d", sent, size, socketfd);
...
process_control_message(void *msg_buf, void *data, size_t sz)
{
	...
	/* Ensuring entire message has been processed */
	if (sz != buf_trav_len) {
		plt_err("Out of %ld bytes %d bytes of msg_buf processed", sz, buf_trav_len);
		rc = -EFAULT;
		goto fail;
	}
...
cnxk_rep_msg_control_thread_launch(struct cnxk_eswitch_dev *eswitch_dev)
{
	...
	rc = rte_ctrl_thread_create(&eswitch_dev->rep_ctrl_msg_thread, name, NULL,
