...
#define RTE_LOGTYPE_BBDEV_OCTEON_EP bbdev_octeon_ep_logtype


/* Helper macro for logging */
#define rte_bbdev_log(level, ...) \
	RTE_LOG_LINE(level, BBDEV_OCTEON_EP, __VA_ARGS__)
...
info_get(struct rte_bbdev *bbdev, struct rte_bbdev_driver_info *dev_info_out)
{
	...
	rte_info->fft_window_width = NULL;
...
exit:
	rte_bbdev_log(ERR, "device %u %s error %i", bbdev->data->dev_id, __func__, msg->status);
	rte_pktmbuf_free(mbuf);
exit1:
...
err:
	if (ret)
		rte_bbdev_log(ERR, "BBDev %u config error %i", bbdev->data->dev_id, ret);
	return ret;
}
...
err:
	if (ret)
		rte_bbdev_log(ERR, "BBDev %u q%u release error %i", bbdev->data->dev_id,
			      q_no, ret);
	return ret;
}
...
err1:		/* Release queue as config failed at target */
		ret = cnxk_ep_bb_queue_release(cnxk_ep_bb_vf, q_no);
err:		rte_bbdev_log(ERR, "BBDev %u q%u config error %i", bbdev->data->dev_id,
				q_no, ret);
	} else
