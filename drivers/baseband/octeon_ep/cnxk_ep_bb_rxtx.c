...
cnxk_ep_bb_dmazone_free(const struct rte_memzone *mz)
{
	...
	if (mz_tmp == NULL) {
		cnxk_ep_bb_err("Memzone %s Not Found", mz->name);
		return;
	}

	ret = rte_memzone_free(mz);
	if (ret)
		cnxk_ep_bb_err("Memzone free failed : ret = %d", ret);
}
...
cnxk_ep_bb_delete_iqs(struct cnxk_ep_bb_device *cnxk_ep_bb_vf, uint32_t iq_no)
{
	...
	if (iq == NULL) {
		cnxk_ep_bb_err("Invalid IQ[%d]", iq_no);
		return -EINVAL;
	}
		     ...
		     unsigned int socket_id)
{
	...
	if (iq->iq_mz == NULL) {
		cnxk_ep_bb_err("IQ[%d] memzone alloc failed", iq_no);
		goto iq_init_fail;
	}
	...
	if (num_descs & (num_descs - 1)) {
		cnxk_ep_bb_err("IQ[%d] descs not in power of 2", iq_no);
		goto iq_init_fail;
	}
	...
	iq->req_list = rte_zmalloc_socket("request_list",
			...
			rte_socket_id());
	if (iq->req_list == NULL) {
		cnxk_ep_bb_err("IQ[%d] req_list alloc failed", iq_no);
		goto iq_init_fail;
	}
...
cnxk_ep_bb_delete_oqs(struct cnxk_ep_bb_device *cnxk_ep_bb_vf, uint32_t oq_no)
{
	...
	if (droq == NULL) {
		cnxk_ep_bb_err("Invalid droq[%d]", oq_no);
		return -EINVAL;
	}
	      ...
	      struct rte_mempool *mpool, unsigned int socket_id)
{
	...
	if (droq->desc_ring_mz == NULL) {
		cnxk_ep_bb_err("OQ:%d desc_ring allocation failed", q_no);
		goto init_droq_fail;
	}
		 ...
		 unsigned int socket_id)
{
	...
	if (droq == NULL) {
		cnxk_ep_bb_err("Droq[%d] Creation Failed", oq_no);
		return -ENOMEM;
	}
	...
	if (cnxk_ep_bb_init_droq(cnxk_ep_bb_vf, oq_no, num_descs, desc_size, mpool,
			     socket_id)) {
		cnxk_ep_bb_err("Droq[%d] Initialization failed", oq_no);
		goto delete_OQ;
	}
...
cnxk_ep_bb_iqreq_delete(struct cnxk_ep_bb_instr_queue *iq, uint32_t idx)
{
	...
	case CNXK_EP_BB_REQTYPE_NORESP_NET:
		 ...
		 * cnxk_ep_bb_dbg("IQ buffer freed at idx[%d]", idx);
		 */
		break;
	...
	default:
		cnxk_ep_bb_info("This iqreq mode is not supported:%d", reqtype);
	}
...
otx_bb_xmit_pkts(void *tx_queue, struct rte_mbuf **pkts, uint16_t nb_pkts)
{
	...
	for (i = 0; i < nb_pkts; i++) {
		...
		cnxk_ep_bb_dbg("Word1 [ihtx]: 0x%016lx", (unsigned long)iqcmd.ih);
		...
		cnxk_ep_bb_dbg("Word4 [irh]: 0x%016lx", (unsigned long)iqcmd.irh);
		  ...
		  uint16_t budget)
{
	...
	for (pkts = 0; pkts < new_pkts; pkts++) {
		...
		if (!oq_pkt) {
			RTE_LOG_DP_LINE(ERR, OCTEON_EP_BBDEV,
