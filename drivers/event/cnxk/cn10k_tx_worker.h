...
{
	return (struct cn10k_eth_txq *)(txq_data[(txq_data[m->port] >> 48) +
...
{
	while ((uint64_t)txq->nb_sqb_bufs_adj <= __atomic_load_n(txq->fc_mem, __ATOMIC_RELAXED))
		;
}
...
{
	return (txq->nb_sqb_bufs_adj - __atomic_load_n((int16_t *)txq->fc_mem, __ATOMIC_RELAXED))
	       << txq->sqes_per_sqb_log2;
}

static __rte_always_inline uint16_t
cn10k_sso_tx_one(struct cn10k_sso_hws *ws, struct rte_mbuf *m, uint64_t *cmd, uint8_t sched_type,
		 const uint64_t *txq_data, const uint32_t flags)
{
	...
	cn10k_nix_xmit_prepare(txq, m, cmd, flags, txq->lso_tun_fmt, &sec, txq->mark_flag,
			       txq->mark_fmt);
	...
	if (flags & NIX_TX_OFFLOAD_SECURITY_F && sec)
		cn10k_nix_prep_sec(m, cmd, &laddr, lmt_addr, &lnum, &loff, &shft, txq->sa_base,
				   flags);
...
static __rte_always_inline uint16_t
cn10k_sso_vwqe_split_tx(struct cn10k_sso_hws *ws, struct rte_mbuf **mbufs, uint16_t nb_mbufs,
			uint64_t *cmd, const uint64_t *txq_data, const uint32_t flags)
{
	...
	for (i = 0; i < nb_mbufs; i++) {
		if (port != mbufs[i]->port ||
			...
			if (count) {
				...
				if (space < count)
					goto done;
				cn10k_nix_xmit_pkts_vector(txq, (uint64_t *)ws, &mbufs[last_idx],
	...
	if (count) {
		txq = (struct cn10k_eth_txq *)(txq_data[(txq_data[port] >> 48) + queue] &
					       (BIT_ULL(48) - 1));
		/* Transmit based on queue depth */
		space = cn10k_sso_sq_depth(txq);
		if (space < count)
			goto done;
		cn10k_nix_xmit_pkts_vector(txq, (uint64_t *)ws, &mbufs[last_idx], count, cmd,
					   flags | NIX_TX_VWQE_F);
...
static __rte_always_inline uint16_t
cn10k_sso_hws_event_tx(struct cn10k_sso_hws *ws, struct rte_event *ev, uint64_t *cmd,
		       const uint64_t *txq_data, const uint32_t flags)
{
	struct cn10k_eth_txq *txq;
	struct rte_mbuf *m;
	...
	if (ev->event_type & RTE_EVENT_TYPE_VECTOR) {
		...
		if (meta & BIT(31)) {
			...
			cn10k_nix_xmit_pkts_vector(txq, (uint64_t *)ws, mbufs + offset, nb_pkts,
	...
	m = ev->mbuf;
	return cn10k_sso_tx_one(ws, m, cmd, ev->sched_type, txq_data, flags);
}

#define T(name, sz, flags)                                                                         \
	uint16_t __rte_hot cn10k_sso_hws_tx_adptr_enq_##name(void *port, struct rte_event ev[],    \
							     uint16_t nb_events);                  \
