...
processx4_step3(struct rte_mbuf *pkt[FWDSTEP], uint16_t dst_port[FWDSTEP])
{
	...
	*p[3] = te[3];
	...
	rfc1812_process((struct rte_ipv4_hdr *)
			...
			pkt[4]->ol_flags & RTE_MBUF_F_RX_IP_CKSUM_MASK);
}
...
process_dst_port(uint16_t *dst_ports, uint16_t nb_elem)
{
	...
	while (nb_elem > 7) {
		...
		vector unsigned short dp;

		dp = (vector unsigned short)vec_splats((short)dst_ports[0]);
		dp1 = *((vector unsigned short *)&dst_ports[i]);
